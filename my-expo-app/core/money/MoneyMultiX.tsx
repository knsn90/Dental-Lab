...
const INK = { 900: '#0A0A0A', 700: '#2C2C2C', 500: '#6B6B6B', 400: '#9A9A9A', 100: '#F5F5F5' };
const DANGER = '#D94B4B';
...
function toneFor(total: number, colorBySign?: boolean): string {
  if (!colorBySign) return INK[900];
...
export function MoneyMultiX({
  ...
  const accent = accentColor ?? INK[700];
  ...
  if (!slices || slices.length === 0) {
    return (
      <View style={style}>
        ...
        <Text style={{ fontFamily: DISPLAY_FONT, fontWeight: '300', fontSize: NUM_SIZE[size], color: INK[400] }}>
          {emptyText}
        </Text>
      </View>
    );
    ...
    return (
      <View style={[{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, alignItems: 'center' }, style]}>
        {slices.map(s => (
          ...
          >
            <Text style={{ fontSize: 10, fontWeight: '700', color: INK[500] }}>
              {CURRENCY_META[s.currency].symbol}
            </Text>
            <Text style={{ fontSize: 11, fontWeight: '600', color: toneFor(s.total, colorBySign) }}>
              {formatCompact(s.total, signed)}
            </Text>
            {showCount ? <Text style={{ fontSize: 9, color: INK[400] }}>· {s.count}</Text> : null}
          </View>
        ))}
      </View>
    );
        ...
        {slices.map(s => (
          <View key={s.currency} style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              {showCount ? (
                <Text style={{ fontSize: 10, color: INK[400], marginTop: 1 }}>{s.count} işlem</Text>
              ) : null}
            </View>
            <Text style={[{ fontFamily: DISPLAY_FONT, fontWeight: '300', fontSize: NUM_SIZE[size === 'lg' ? 'md' : size], letterSpacing: -0.4, color: toneFor(s.total, colorBySign) }, numberStyle]}>
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                <Text style={{ fontSize: 12, fontWeight: '700', color: INK[500] }}>
                  {CURRENCY_META[s.currency].symbol}
                </Text>
                <Text style={{ fontSize: 10, fontWeight: '700', color: INK[400], letterSpacing: 0.5 }}>
                  {s.currency}
                </Text>
              </View>
              {showCount ? (
            ...
            <Text
              numberOfLines={1}
              style={[{ fontFamily: DISPLAY_FONT, fontWeight: '300', fontSize: NUM_SIZE[size], letterSpacing: -0.6, color: toneFor(s.total, colorBySign) }, numberStyle]}
            >
              {formatMoney(s.total, s.currency, { fractionDigits, signed })}
            </Text>
          </View>
...
function SectionLabel({ label, count, accent }: { label: string; count?: number; accent?: string }) {
  return (
    <View style={{ flexDirection: 'row', alignItems: 'baseline', gap: 6 }}>
      <Text style={{ fontSize: 11, fontWeight: '700', color: INK[400], letterSpacing: 1, textTransform: 'uppercase' }}>
        {label}
      </Text>
      {count && count > 1 ? (
        <Text style={{ fontSize: 10, fontWeight: '600', color: accent ?? INK[500] }}>
          {count} para birimi
        </Text>
      ) : null}
    </View>
...
function CurrencyBadge({ currency }: { currency: Currency }) {
  return (
    <View style={{ width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.04)' }}>
      <Text style={{ fontSize: 13, fontWeight: '700', color: INK[500] }}>
