/**
 * RegisterClinicScreen — Klinik / Poliklinik (çok hekimli) kayıt formu
 *
...
import {
  ...
  AlertCircle, ChevronLeft, Building2, User, Phone, Users,
...
function InputField({
  ...
  error, rightElement, icon, multiline,
}: {
  label: string; value: string; onChangeText: (v: string) => void;
  placeholder?: string; secureTextEntry?: boolean; keyboardType?: 'default' | 'email-address' | 'phone-pad';
  autoCapitalize?: 'none' | 'sentences'; error?: string; rightElement?: React.ReactNode;
          ...
          return (
            <Pressable
              ...
              style={{
                ...
                backgroundColor: active ? GREEN_DEEP : '#FFFFFF',
                borderWidth: 1, borderColor: active ? GREEN_DEEP : 'rgba(0,0,0,0.08)',
                alignItems: 'center',
              }}
            >
              <Text style={{ fontSize: 13, fontWeight: '600', color: active ? '#FFFFFF' : DS.ink[500] }}>
                {o.label}
              </Text>
            </Pressable>
          );
...
export function RegisterClinicScreen() {
  ...
  const handleRegister = async () => {
    ...
    const { data, error } = await signUpClinic({
    ...
    <Animated.View style={{ transform: [{ translateX: shakeX }] }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 4 }}>
        <View style={{
          ...
          backgroundColor: `${GREEN_DEEP}18`, alignItems: 'center', justifyContent: 'center',
        }}>
          <Building2 size={14} color={GREEN_DEEP} strokeWidth={2} />
        </View>
        <Text style={{ fontSize: 11, color: GREEN_DEEP, letterSpacing: 1.1, textTransform: 'uppercase', fontWeight: '600' }}>
          Klinik · Çok Hekimli
        </Text>
      </View>
      ...
      <Text style={{ fontSize: 13, color: DS.ink[500], lineHeight: 19, marginBottom: 24 }}>
        Klinik bilgileri + yönetici hekim hesabı oluşturulur. Onay sonrası hekim davet edebilirsiniz.
      </Text>
      ...
      {/* ── Klinik Bilgileri ── */}
      <SectionDivider icon={<Building2 size={12} color={GREEN} strokeWidth={2} />} label="Klinik Bilgileri" />

      <InputField label="Klinik / Poliklinik Adı *" value={form.clinic_name} onChangeText={set('clinic_name')}
        placeholder="Yılmaz Dental Klinik" error={errors.clinic_name}
      ...
      <InputField label="Klinik Telefon" value={form.clinic_phone} onChangeText={set('clinic_phone')}
        placeholder="0212 000 00 00" keyboardType="phone-pad"
        icon={<Phone size={14} color={DS.ink[400]} strokeWidth={1.8} />} />

      {/* ── Yönetici Hekim ── */}
      <SectionDivider icon={<User size={12} color={GREEN} strokeWidth={2} />} label="Yönetici Hekim" />

      <View style={{
        flexDirection: 'row', alignItems: 'center', gap: 8,
        padding: 10, borderRadius: 10, backgroundColor: `${GREEN}12`, marginBottom: 14,
      }}>
        <Users size={13} color={GREEN} strokeWidth={1.8} />
        <Text style={{ fontSize: 11, color: DS.ink[700], flex: 1, lineHeight: 16 }}>
          Onay sonrası bu hesaptan diğer hekimleri davet edebilirsiniz.
        </Text>
      </View>
      ...
      <InputField label="Cep Telefon *" value={form.phone} onChangeText={set('phone')}
      ...
      <Animated.View style={{ transform: [{ scale: btnScale }], marginTop: 10 }}>
        ...
        >
          {loading ? (
            ...
            <>
              <Text style={{ fontSize: 14, fontWeight: '500', color: '#FFF' }}>Klinik Kaydet</Text>
  ...
  if (isDesktop) {
    return (
      <View style={{ flex: 1, flexDirection: 'row', backgroundColor: GREEN_DEEP }}>
        <View style={{ flex: 1, padding: 48, justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
          ...
          <View>
            ...
            <Text style={{
              ...
              Kurumsal{'\n'}
              <Text style={{ color: GREEN }}>klinik</Text>{'\n'}
              yönetimi.
            </Text>
            <Text style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', maxWidth: 420, lineHeight: 21 }}>
              Kliniğinizi kaydedin, hekimlerinizi davet edin. Merkezi sipariş ve fatura yönetimi.
            </Text>
          </View>
        ...
        <View style={{ width: 460, backgroundColor: KREM, borderRadius: 28, borderTopRightRadius: 0, borderBottomRightRadius: 0, padding: 40, marginTop: 24, marginBottom: 24, marginLeft: 24 }}>
    ...
    <View style={{ flex: 1, backgroundColor: GREEN_DEEP }}>
      <View style={{
        ...
        <View style={{
          ...
          <Building2 size={24} color="#FFFFFF" strokeWidth={1.8} />
        </View>
        <Text style={{ fontSize: 16, fontWeight: '600', color: '#FFF', letterSpacing: 1.5, marginBottom: 4 }}>
          KLİNİK KAYDI
        </Text>
        <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>Çok hekimli kurum</Text>
      </View>
