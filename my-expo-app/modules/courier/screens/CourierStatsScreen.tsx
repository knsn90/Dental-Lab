...
import {
  View, Text, ScrollView, Pressable, useWindowDimensions, RefreshControl, Platform,
} from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import {
  Truck, Timer, Navigation, Check, Star, BarChart3, TrendingUp,
  Calendar, Award, Activity, Bell,
      ...
      <View style={{
        ...
        <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          ...
          <View>
            ...
            <Text style={{ fontSize: 10, color: DS.ink[500] }}>Performansım</Text>
          </View>
        </View>
      ...
      <View style={{ paddingHorizontal: 16, gap: 16 }}>
        ...
        <View style={{ borderRadius: 28, backgroundColor: TH.bg, padding: 14, borderWidth: 1, borderColor: TH.bgDeep }}>
          <View style={{
            ...
            <Text style={{ fontSize: 11, fontWeight: '600', letterSpacing: 1.2, textTransform: 'uppercase', color: DS.ink[500] }}>
              {t('courier.stats.courierPerformance')}
            </Text>
            <Text style={{ ...DISPLAY, fontSize: 36, color: DS.ink[900], letterSpacing: -1, lineHeight: 40, marginTop: 2 }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
          ...
          <PremiumKPI size="sm" icon={Timer}    label={t('courier.kpi.avgTime')}      value={`${stats.avgMin} dk`} sub={stats.fastest > 0 ? t('courier.stats.fastest', { count: stats.fastest }) : '—'}    accent={SUCCESS} />
        ...
        <View style={{ borderRadius: 18, backgroundColor: '#FFF', borderWidth: 1, borderColor: DS.ink[200], padding: 16, gap: 12 }}>
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between' }}>
            <View>
              <Text style={{ fontSize: 10, fontWeight: '700', letterSpacing: 0.8, textTransform: 'uppercase', color: DS.ink[500] }}>{t('courier.stats.last7Days')}</Text>
              <Text style={{ ...DISPLAY, fontSize: 22, color: DS.ink[900], letterSpacing: -0.5, marginTop: 2 }}>{t('courier.stats.dailyTrend')}</Text>
            </View>
            <Text style={{ fontSize: 11, color: DS.ink[500] }}>{last7Days.reduce((s: number, d: any) => s + d.count, 0)} {t('courier.stats.totalMetric')}</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 6, height: 100, marginTop: 4 }}>
            {last7Days.map((d: any, i: number) => (
              <View key={i} style={{ flex: 1, alignItems: 'center', gap: 5 }}>
                ...
                <Text style={{ fontSize: 9, color: DS.ink[500], fontWeight: '600' }}>{d.day}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        {/* Son Teslimatlar */}
        <View>
          <Text style={{ ...DISPLAY, fontSize: 18, color: DS.ink[900], letterSpacing: -0.3, marginBottom: 12 }}>{t('courier.stats.recentDeliveries')}</Text>
          {filtered.length === 0 ? (
            <View style={{ padding: 24, alignItems: 'center', gap: 8, borderRadius: 14, borderWidth: 1, borderColor: DS.ink[200], backgroundColor: '#FFF' }}>
              <Calendar size={20} color={DS.ink[400]} />
              <Text style={{ fontSize: 13, color: DS.ink[500] }}>{t('courier.stats.noPeriodDeliveries2')}</Text>
            </View>
          ) : (
            <View style={{ gap: 8 }}>
              {filtered.slice(0, 10).map((d: CourierDelivery) => {
                const dur = d.picked_up_at && d.delivered_at
                  ...
                  : null;
                return (
                  <View key={d.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, borderRadius: 14, backgroundColor: '#FFF', borderWidth: 1, borderColor: DS.ink[100] }}>
...
function DesktopView({ profile, filtered, stats, loading, range, setRange, onRefresh, last7Days, maxDayCount }: any) {
  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between' }}>
        <View>
          <Text style={{ fontSize: 13, fontWeight: '500', color: DS.ink[500] }}>{greeting(t)}, {profile?.full_name?.split(' ')[0] ?? 'Kurye'}</Text>
          <Text style={{ ...DISPLAY, fontSize: 30, color: DS.ink[900], letterSpacing: -0.9, lineHeight: 34, marginTop: 2 }}>{t('courier.stats.myPerformance')}</Text>
          <Text style={{ fontSize: 12, color: DS.ink[500], marginTop: 4 }}>{t('courier.stats.description')}</Text>
        </View>
        <RangePillBar value={range} onChange={setRange} />
      </View>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 32, flexWrap: 'wrap' }}>
          <View style={{ flex: 1, minWidth: 280 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 18, paddingVertical: 12, borderRadius: 999, backgroundColor: DS.ink[900], marginTop: 24, alignSelf: 'flex-start' }}>
              ...
              <Text style={{ fontSize: 13, fontWeight: '700', color: '#FFF' }}>{t('courier.stats.detailedReport')}</Text>
            </View>
          </View>
          <View style={{
      ...
      <View style={{ flexDirection: 'row', gap: 12 }}>
        ...
        <PremiumKPI size="lg" icon={Timer}    label={t('courier.stats.kpiAvgTime')}     value={`${stats.avgMin} dk`}                          sub={stats.fastest > 0 ? t('courier.stats.fastest', { count: stats.fastest }) : '—'}   accent={SUCCESS}
          trend={stats.fastest > 0 && stats.avgMin < 20 ? { value: 'Hızlı', tone: 'up' } : undefined} />
      ...
      <View style={{ flexDirection: 'row', gap: 16 }}>
        <Card padding={20} style={{ flex: 1 }}>
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between' }}>
            <View>
              <Text style={{ fontSize: 10, fontWeight: '700', letterSpacing: 0.8, textTransform: 'uppercase', color: DS.ink[500] }}>{t('courier.stats.chart1Last7Days')}</Text>
              <Text style={{ ...DISPLAY, fontSize: 22, color: DS.ink[900], letterSpacing: -0.5, marginTop: 2 }}>{t('courier.stats.dailyDeliveries')}</Text>
            </View>
            <Text style={{ fontSize: 11, color: DS.ink[500] }}>{last7Days.reduce((s: number, d: any) => s + d.count, 0)} {t('courier.stats.totalMetric')}</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 8, height: 140, marginTop: 20 }}>
            {last7Days.map((d: any, i: number) => (
              <View key={i} style={{ flex: 1, alignItems: 'center', gap: 6 }}>
                ...
                <Text style={{ fontSize: 10, color: DS.ink[500], fontWeight: '600' }}>{d.day}</Text>
              </View>
            ))}
          </View>
        </Card>
        ...
        <Card padding={20} style={{ flex: 1 }}>
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between' }}>
            <View>
              <Text style={{ fontSize: 10, fontWeight: '700', letterSpacing: 0.8, textTransform: 'uppercase', color: DS.ink[500] }}>{t('courier.stats.efficiency')}</Text>
              <Text style={{ ...DISPLAY, fontSize: 22, color: DS.ink[900], letterSpacing: -0.5, marginTop: 2 }}>{t('courier.stats.timeDistribution')}</Text>
            </View>
              ...
              return (
                <View key={i} style={{ gap: 4 }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={{ fontSize: 12, fontWeight: '600', color: DS.ink[700] }}>{row.label}</Text>
                    <Text style={{ fontSize: 12, fontWeight: '700', color: DS.ink[900] }}>{row.value}</Text>
                  </View>
                  <View style={{ height: 8, borderRadius: 999, backgroundColor: DS.ink[100], overflow: 'hidden' }}>
        ...
        <Card padding={0}>
          <View style={{ flexDirection: 'row', paddingHorizontal: 16, paddingVertical: 10, backgroundColor: DS.ink[50], borderBottomWidth: 1, borderBottomColor: DS.ink[100] }}>
            {[t('courier.table.date'), t('courier.table.customer'), t('courier.table.address'), t('courier.table.duration'), t('courier.table.status')].map((h, i) => (
              <Text key={h} style={{ flex: i === 2 ? 2 : 1, fontSize: 10, fontWeight: '700', color: DS.ink[500], textTransform: 'uppercase', letterSpacing: 0.6 }}>{h}</Text>
            ))}
          </View>
          {filtered.length === 0 ? (
            <View style={{ paddingVertical: 40, alignItems: 'center', gap: 8 }}>
              <Calendar size={24} color={DS.ink[300]} />
              <Text style={{ fontSize: 13, color: DS.ink[500] }}>{t('courier.stats.emptyPeriod')}</Text>
            </View>
            ...
            return (
              <View key={d.id} style={{
                ...
                <Text style={{ flex: 1, fontSize: 11, color: DS.ink[500] }}>
    ...
    <View style={{ flexDirection: 'row', alignSelf: 'flex-start', padding: 4, borderRadius: 999, backgroundColor: 'rgba(0,0,0,0.05)' }}>
      {opts.map(o => {
        ...
        return (
          ...
          >
            <Text style={{ fontSize: 12, fontWeight: active ? '700' : '500', color: active ? DS.ink[900] : DS.ink[500] }}>{o.label}</Text>
          </Pressable>
        );
      })}
    </View>
  ...
  return (
    <View style={{ flex: 1, minWidth: '46%', backgroundColor: '#FFF', borderRadius: 16, borderWidth: 1, borderColor: DS.ink[200], padding: 14, gap: 6 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
        ...
        <Text style={{ fontSize: 9, fontWeight: '700', letterSpacing: 0.8, textTransform: 'uppercase', color: DS.ink[500], flex: 1 }}>{label}</Text>
      </View>
      <Text style={{ ...DISPLAY, fontSize: 22, letterSpacing: -0.7, lineHeight: 26, color: DS.ink[900] }}>{value}</Text>
      {sub && <Text style={{ fontSize: 10, color: DS.ink[500] }} numberOfLines={1}>{sub}</Text>}
    </View>
  );
}
...
function KPIDesktop({ icon: Icon, label, value, sub, accent }: { icon: any; label: string; value: string; sub?: string; accent: string }) {
  return (
    <View style={{ flex: 1, backgroundColor: '#FFF', borderRadius: 16, borderWidth: 1, borderColor: DS.ink[200], padding: 18, gap: 8 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        ...
        <Text style={{ fontSize: 10, fontWeight: '700', letterSpacing: 0.8, textTransform: 'uppercase', color: DS.ink[500], flex: 1 }}>{label}</Text>
      </View>
      <Text style={{ ...DISPLAY, fontSize: 26, letterSpacing: -0.7, lineHeight: 30, color: DS.ink[900] }}>{value}</Text>
      {sub && <Text style={{ fontSize: 11, color: DS.ink[500] }} numberOfLines={1}>{sub}</Text>}
    </View>
  );
}
