...
import { Star, X, ImagePlus, Upload } from 'lucide-react-native';
...
function PhotoGrid({ label, hint, list, signed, readOnly, accent, uploading, onAdd, onRemove }: {
  ...
  return (
    <View style={{ gap: 8 }}>
      <Text style={{ fontSize: 11, fontWeight: '700', color: DS.ink[400], textTransform: 'uppercase', letterSpacing: 0.8 }}>{label}</Text>
      {hint ? <Text style={{ fontSize: 11, color: DS.ink[400], marginTop: -4 }}>{hint}</Text> : null}
          ...
          <Pressable onPress={onAdd} disabled={uploading} style={{ width: 96, height: 116, borderRadius: 12, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: hexA(accent, 0.4), borderStyle: 'dashed', overflow: 'hidden', cursor: 'pointer' as any }}>
            ...
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', gap: 6, paddingBottom: 16 }}>
              {uploading ? <ActivityIndicator color={accent} /> : <>
                ...
                <Text style={{ fontSize: 11, fontWeight: '600', color: DS.ink[500] }}>Foto Ekle</Text>
              </>}
            </View>
            {!uploading && (
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: 'rgba(15,20,18,0.55)', justifyContent: 'center', alignItems: 'center', padding: 20 }}>
        <View style={{
          width: '100%', maxWidth: isWide ? 880 : 480, maxHeight: '90%', backgroundColor: '#FFFFFF', borderRadius: 24, overflow: 'hidden',
          // @ts-ignore web
          ...
          <ScrollView contentContainerStyle={{ padding: 16 }} showsVerticalScrollIndicator={false}>
           <View style={{ flexDirection: isWide ? 'row' : 'column', gap: 16, alignItems: 'flex-start' }}>
            ...
            <View style={{ alignItems: 'center', gap: 6, backgroundColor: hexA(accent, 0.06), borderRadius: 16, paddingVertical: 12, paddingHorizontal: 14, borderWidth: 1, borderColor: hexA(accent, 0.12) }}>
              <Text style={{ fontSize: 10, fontWeight: '700', color: DS.ink[500], textTransform: 'uppercase', letterSpacing: 1 }}>Genel Puan</Text>
              <StarRow value={overall} onChange={setOverall} color={accent} size={30} readOnly={readOnly} />
              <Text style={{ fontSize: 13, fontWeight: '700', color: overall > 0 ? accent : DS.ink[400] }}>
            ...
            <View style={{ gap: 6 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 2 }}>
                <Text style={{ fontSize: 11, fontWeight: '700', color: DS.ink[400], textTransform: 'uppercase', letterSpacing: 0.8 }}>Detaylı Değerlendirme</Text>
                {!readOnly && <Text style={{ fontSize: 11, color: DS.ink[400] }}>{ratedDims}/{REVIEW_DIMENSIONS.length}</Text>}
              </View>
              {REVIEW_DIMENSIONS.map(d => {
                ...
                return (
                  <View key={d.key} style={{
                    ...
                    backgroundColor: v > 0 ? hexA(accent, 0.05) : DS.ink[50],
                    borderRadius: 12, paddingVertical: 7, paddingHorizontal: 12,
                    borderWidth: 1, borderColor: v > 0 ? hexA(accent, 0.14) : 'transparent',
                  }}>
                    <View style={{ flex: 1, minWidth: 0 }}>
                      <Text style={{ fontSize: 13, fontWeight: '600', color: DS.ink[900] }}>{d.label}</Text>
                      <Text style={{ fontSize: 10, color: DS.ink[400], marginTop: 1 }}>{d.hint}</Text>
                    </View>
            ...
            {/* ── Yorum ── */}
            <View style={{ gap: 8 }}>
              <Text style={{ fontSize: 11, fontWeight: '700', color: DS.ink[400], textTransform: 'uppercase', letterSpacing: 0.8 }}>Yorum {readOnly ? '' : '· opsiyonel'}</Text>
              {readOnly ? (
                <Text style={{ fontSize: 14, color: comment ? DS.ink[800] : DS.ink[400], lineHeight: 20 }}>{comment || 'Yorum yok'}</Text>
              ) : (
                <TextInput
                  value={comment}
                  ...
                  multiline
                  style={{
                    minHeight: 64, fontSize: 14, color: DS.ink[900], textAlignVertical: 'top',
                    backgroundColor: DS.ink[50], borderRadius: 14, padding: 12,
            ...
            {existing?.lab_reply ? (
              <View style={{ backgroundColor: hexA(accent, 0.07), borderRadius: 14, padding: 14, gap: 4, borderWidth: 1, borderColor: hexA(accent, 0.14) }}>
                ...
                <Text style={{ fontSize: 13, color: DS.ink[800], lineHeight: 19 }}>{existing.lab_reply}</Text>
              </View>
            ) : null}
          ...
          {/* ── Footer ── */}
          {!readOnly && (
            <View style={{ flexDirection: 'row', gap: 10, paddingHorizontal: 20, paddingVertical: 12, borderTopWidth: 1, borderTopColor: DS.ink[100], backgroundColor: '#FFFFFF' }}>
              <Pressable onPress={onClose} style={{ flex: 1, height: 44, borderRadius: 999, alignItems: 'center', justifyContent: 'center', backgroundColor: DS.ink[100], cursor: 'pointer' as any }}>
                <Text style={{ fontSize: 14, fontWeight: '600', color: DS.ink[700] }}>Vazgeç</Text>
              </Pressable>
              <Pressable
                onPress={handleSave}
                ...
                style={{
                  ...
                  backgroundColor: canSave ? T.primaryDeep : DS.ink[200],
