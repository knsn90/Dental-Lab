-- ============================================================
-- 20260512030000 — Station default technician + auto-assign on
-- stage activation
--
-- 1) lab_stations.default_technician_id — istasyonun varsayılan
--    teknisyeni. NULL ise atama yapılmaz, manager elle atar.
-- 2) complete_stage_simple — sıradaki aşamayı aktive ederken
--    o aşamada technician_id NULL ise istasyonun default'unu kullan.
-- 3) triage_order — ilk aktif aşamada da aynı kural geçerli.
-- ============================================================
...
COMMENT ON COLUMN public.lab_stations.default_technician_id IS
  'İstasyona iş geldiğinde otomatik atanan teknisyen (opsiyonel).';

-- 2) complete_stage_simple — sıradaki aşamada auto-assign
...
BEGIN
  ...
  LOOP
    ...
    SELECT os.id, ls.auto_progress, os.sequence_order, ls.default_technician_id
      INTO v_next_id, v_auto_prog, v_next_seq, v_default_tech
    ...
    IF v_auto_prog THEN
      UPDATE public.order_stages
       ...
       WHERE id = v_next_id;
    ELSE
      -- Manuel aşama: aktif et + auto-assign (technician_id boşsa default'a)
      UPDATE public.order_stages
         SET status='aktif',
...
-- 3) triage_order — ilk aktif aşamada default tech ata
...
BEGIN
  ...
  LOOP
    ...
    -- Manuel teknisyen verilmediyse istasyon default'unu çek
...
-- 4) Mevcut aktif ama atanmamış aşamaları default ile doldur
UPDATE public.order_stages os
   SET technician_id = ls.default_technician_id,
       assigned_at = COALESCE(os.assigned_at, NOW())
  FROM public.lab_stations ls
 WHERE os.station_id = ls.id
   AND os.status = 'aktif'
   AND os.technician_id IS NULL
   AND ls.default_technician_id IS NOT NULL;
