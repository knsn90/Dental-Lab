-- Satır bazlı KDV desteği:
--   • stock_movements'a vat_rate ve vat_amount kolonları
--   • create_purchase_invoice RPC artık her satırın kendi vat_rate'i ile çalışır
--   • Satırda vat_rate yoksa p_vat_rate (header default) kullanılır
--   • Header'daki subtotal/vat_amount/total satırların TOPLAMI olarak hesaplanır
--   • Header.vat_rate: tüm satırlar aynıysa o oran, mixed ise -1 ("Karışık")
...
CREATE OR REPLACE FUNCTION public.create_purchase_invoice(
  ...
  p_due_date        date,
  p_currency        text,
  p_vat_rate        numeric,  -- DEFAULT — satırda vat_rate yoksa bu kullanılır
...
BEGIN
  ...
  -- Header vat_rate: tüm satırlarda aynıysa o, mixed ise NULL
  ...
  IF v_distinct_vat_rates = 1 THEN
    SELECT COALESCE((l->>'vat_rate')::numeric, p_vat_rate, 0)
      ...
      LIMIT 1;
  ELSE
    v_header_vat_rate := NULL;  -- "Karışık" — UI bunu okuyup gösterir
  END IF;
