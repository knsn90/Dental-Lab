-- ════════════════════════════════════════════════════════════════════════════
-- FIX: satın alma faturası → ÇİFT gider kaydı (ve "sil → ikisi de gider").
--
--   purchase_invoices üzerindeki trg_purchase_invoice_to_expense (AFTER INSERT)
--   trigger'ı zaten bir expenses satırı yaratıyor. Trigger NOT EXISTS guard'ına
--   sahip ("RPC eklediyse ekleme") ama AFTER INSERT olduğu için fatura insert'inden
--   HEMEN sonra, RPC'nin manuel expense insert'inden ÖNCE çalışıyor → guard boş
--   bulur → ekler. Ardından RPC ikinci (guard'sız) expense'i ekliyor → 2 kayıt
--   (ikisi de aynı purchase_invoice_id → faturayı silince ikisi de gider).
--
--   ÇÖZÜM: tek otorite trigger olsun. create_purchase_invoice (her iki overload)
--   içindeki manuel "INSERT INTO public.expenses ..." bloğu kaldırılır. Trigger
--   hem INSERT'te oluşturur hem UPDATE'te senkron tutar (RPC bunu yapmıyordu).
--   Stok / stock_movements / supplier_transactions akışı aynen korunur.
-- ════════════════════════════════════════════════════════════════════════════

-- ─── Overload 1: 12 argüman (exchange_rate yok, legacy) ──────────────────────
...
DECLARE
  ...
  v_line_vat_amount  numeric;
  v_distinct_vat_rates int;
  v_header_vat_rate  numeric;
BEGIN
  ...
  LOOP
    ...
    v_vat_amount := v_vat_amount + v_line_vat_amount;
  END LOOP;
  ...
  SELECT COUNT(DISTINCT COALESCE((l->>'vat_rate')::numeric, p_vat_rate, 0))
    INTO v_distinct_vat_rates
    FROM jsonb_array_elements(p_lines) l;

  IF v_distinct_vat_rates = 1 THEN
    SELECT COALESCE((l->>'vat_rate')::numeric, p_vat_rate, 0)
      INTO v_header_vat_rate
      FROM jsonb_array_elements(p_lines) l
      LIMIT 1;
  ELSE
    v_header_vat_rate := NULL;
  END IF;
  ...
  INSERT INTO public.purchase_invoices (
    ...
    v_subtotal, v_header_vat_rate, v_vat_amount, v_total,
  ...
  LOOP
    ...
    IF v_kind = 'equipment' THEN
      ...
      FOR i IN 1..GREATEST(1, FLOOR(v_qty)::int) LOOP
        INSERT INTO public.equipment (
          lab_id, name, brand, model, category, status, purchase_date, notes
        ) VALUES (
          p_lab_id, TRIM(v_line->>'item_name'), v_brand_name,
          NULLIF(TRIM(v_line->>'model'), ''), v_eq_category, 'active',
          p_invoice_date,
    ...
    INSERT INTO public.stock_movements (
      ...
      v_line_vat_rate, v_line_vat_amount
    );
  ...
  -- NOT: expenses satırını trg_purchase_invoice_to_expense trigger'ı oluşturur.
  -- (Manuel INSERT kaldırıldı — çift gider kaydını engellemek için.)

  RETURN v_invoice_id;
END;
$function$;

-- ─── Overload 2: 13 argüman (p_exchange_rate — client bunu kullanır) ─────────
CREATE OR REPLACE FUNCTION public.create_purchase_invoice(
  p_lab_id uuid, p_supplier_id uuid, p_supplier_name text, p_invoice_number text,
  p_invoice_date date, p_due_date date, p_currency text, p_vat_rate numeric,
  p_payment_method text, p_invoice_file_url text, p_notes text, p_lines jsonb,
...
BEGIN
  ...
  ELSE
    v_rate := public.get_currency_rate(p_lab_id, p_currency, v_base_currency, p_invoice_date);
    IF v_rate IS NULL AND p_currency <> v_base_currency THEN
      RAISE EXCEPTION 'Currency rate not defined for % -> % on % — fatura kurunu manuel gir', p_currency, v_base_currency, p_invoice_date;
    END IF;
    IF v_rate IS NULL THEN v_rate := 1; END IF;
  END IF;
