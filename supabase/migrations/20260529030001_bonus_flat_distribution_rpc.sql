-- ─────────────────────────────────────────────────────────────────────────────
-- Bonus Engine — flat_per_member dağıtımı için pool calc patch
-- Pool tutarı = aktif kişi sayısı × flat_amount_per_member (hata satırı olmayanlar)
-- Dağıtım = her kişiye flat_amount_per_member
-- ─────────────────────────────────────────────────────────────────────────────

-- Var olan fonksiyonu replace ediyoruz; sadece pool block'unu genişlettik.
...
BEGIN
  ...
  -- 3) Çalışanları döngüye al — her biri için bireysel hesaplama
  FOR v_emp IN
  ...
  LOOP
    ...
    -- birim & puan: tooth_numbers toplamı + difficulty multiplier
    WITH wo AS (
    ...
    -- kalite — remake oranı
    SELECT COUNT(*) FILTER (WHERE wo.is_remake = true),
           COUNT(*)
      INTO v_remakes, v_total_jobs
      FROM work_orders wo
     WHERE wo.created_at >= v_quality_start AND wo.created_at <= v_period_end + interval '1 day';
  ...
  -- 4) POOL — pool veya hybrid modunda
  IF v_policy.mode IN ('pool', 'hybrid') THEN

    -- ─── DAĞITIM YÖNTEMİ: flat_per_member ──────────────────────────────
    ...
    -- ─── DAĞITIM YÖNTEMİ: oran tabanlı (eski mantık) ───────────────────
    ELSE
